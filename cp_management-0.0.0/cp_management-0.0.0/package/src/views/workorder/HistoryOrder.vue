...
import {getOrderHistoryService,deleteOrderHistoryService,getUserTotalService} from '@/api/workorder.js'
const getTotal=async()=>{
  let result =await getUserTotalService();
  total.value=result.data
}
getTotal();
const onCurrentChange =async(num)=>{
    pageNum.value=num
    let result = await getOrderHistoryService(pageNum.value);
